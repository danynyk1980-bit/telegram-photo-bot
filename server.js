...
bot.setMyCommands([
  ...
  {
    command: '/reset',
    description: '🔄 Сбросить состояние бота'
  }
]);
...
bot.onText(/\/reset/, (msg) => {
  ...
  bot.sendMessage(chatId, 
    '✅ Состояние бота сброшено! Теперь вы можете отправлять фотографии.\n\nНажмите /start для просмотра основной информации или просто отправьте фото.',
    {
      reply_markup: {
        inline_keyboard: [
...
bot.on('message', (msg) => {
  ...
  // Для любых текстовых сообщений показываем инструкцию
  bot.sendMessage(chatId,
    `📸 *Бот для сбора фотографий PR-Ёлки*\n\nЯ принимаю фотографии для юбилейного ролика к 10-летию конференции.\n\nПросто отправьте фото в этот чат!`,
    {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '📸 Отправить фото', callback_data: 'send_photo' }],
          [{ text: '🔄 Сбросить состояние', callback_data: 'reset' }],
          [{ text: '🌐 Узнать о конференции', url: 'https://pr.dp.ru/' }]
        ]
      }
    }
...
bot.on('callback_query', (callbackQuery) => {
  ...
  if (callbackQuery.data === 'reset') {
    bot.sendMessage(chatId, 
      '✅ Состояние сброшено! Теперь вы можете отправлять фотографии.\n\nПросто пришлите фото в чат 📸',
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: '📸 Отправить фото', callback_data: 'send_photo' }]
